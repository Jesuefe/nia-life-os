...
Schema: {"goals":[{"id":"g_TS","title":"","category":"health|finance|career|personal|relationship","progress":0}],"habits":[{"id":"h_TS","title":"","frequency":"daily|weekly","streak":0}],"reminders":[{"id":"r_TS","title":"","when":"","recurring":false,"done":false,"whatsapp":false}],"markDone":[],"notes":[],"userName":null,"age":null,"marital":null,"occupation":null}
Rules: markDone = IDs of items user said they completed. Only NEW items. null for unchanged fields. If user says "remind me on WhatsApp", set whatsapp:true on that reminder.`;
...
function useMemory() {
  ...
  const [mem, setMem] = useState(() => { try { const s=localStorage.getItem("nia_v3"); const parsed=s?JSON.parse(s):{}; return {...def,...parsed,health:{...HEALTH_DEF,...(parsed.health||{})}}; } catch { return def; } });
  const save = useCallback((u) => setMem(p => { const n={...p,...u}; try{localStorage.setItem("nia_v3",JSON.stringify(n));}catch{} return n; }), []);
  return [mem, save];
}
function useMsgs() {
  const [msgs, setMsgs] = useState(() => { try{const s=localStorage.getItem("nia_msgs_v3");return s?JSON.parse(s):[];}catch{return [];} });
...
// ─── CHAT VIEW ───────────────────────────────────────────────────────────────
function ChatView({mem,save,msgs,addMsg,onOpenWA}) {
  const [input,setInput]=useState("");
  const [loading,setLoading]=useState(false);
  const bottomRef=useRef(null);
...
// ─── NAV ─────────────────────────────────────────────────────────────────────
const NAV=[{id:"chat",icon:"chat",label:"Chat"},{id:"morning",icon:"sun",label:"Briefing"},{id:"goals",icon:"target",label:"Goals"},{id:"habits",icon:"loop",label:"Habits"},{id:"reminders",icon:"bell",label:"Reminders"},{id:"health",icon:"heart",label:"Health"},{id:"brainstorm",icon:"lightbulb",label:"Research"},{id:"memory",icon:"brain",label:"Memory"}];
...
function AuthenticatedApp({user}) {
  ...
  useEffect(()=>{
    const load=async()=>{
      ...
      if(m&&(m.goals?.length||m.habits?.length||m.notes?.length)) save({goals:m.goals||[],habits:m.habits||[],reminders:m.reminders||[],notes:m.notes||[],habitLogs:m.habit_logs||{},health:m.health||{},whatsappLogs:m.whatsapp_logs||[]});
      const {data:p}=await supabase.from("profiles").select("*").eq("id",user.id).single();
      if(p) save({userName:p.name||mem.userName,age:p.age||mem.age,marital:p.marital_status||mem.marital,occupation:p.occupation||mem.occupation,whatsappNumber:p.whatsapp_number||mem.whatsappNumber,onboarded:p.onboarded||mem.onboarded,plan:p.plan||"free"});
    };
    load();
  },[]);
