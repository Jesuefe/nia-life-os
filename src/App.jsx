...
const HEALTH_DEF = { enabled:false, waterGoal:2.5, waterToday:0, waterLog:[], medicationEnabled:false, medications:[], medLogs:{}, sleepEnabled:false, sleepTarget:"23:00", sleepLog:[], mealsEnabled:false, mealLog:[], nudgingStyle:"gentle", quietStart:22, quietEnd:7, bpLog:[], sugarLog:[] };

function getReminderStatus(reminder) {
  // Parse when field and determine if overdue/today/upcoming
  const when = (reminder.when||"").toLowerCase();
  const now = new Date();
  const today = now.toDateString();
  const todayWords = ["today","tonight","this morning","this evening","this afternoon","now"];
  const overdueWords = ["yesterday","overdue","last week","last month"];
  if(todayWords.some(w=>when.includes(w))) return "today";
  if(overdueWords.some(w=>when.includes(w))) return "overdue";
  // Check day names
  const days=["sunday","monday","tuesday","wednesday","thursday","friday","saturday"];
  const dayMatch=days.find(d=>when.includes(d));
  if(dayMatch){
    const targetDay=days.indexOf(dayMatch);
    const currentDay=now.getDay();
    if(targetDay===currentDay) return "today";
    if(targetDay<currentDay) return "overdue";
  }
  return "upcoming";
}
...
- Return ONLY the JSON, no explanation`;
          ...
          if(p.health){
            ...
            if(p.health.sugarLog?.length){
              newHealth.sugarLog=[...(curHealth.sugarLog||[]),...p.health.sugarLog.map(r=>({...r,ts:r.ts||Date.now(),date:r.date||today}))];
            }
            // Sleep: append
            if(p.health.sleepLog?.length){
              newHealth.sleepLog=[...(curHealth.sleepLog||[]),...p.health.sleepLog.map(r=>({...r,ts:r.ts||Date.now(),date:r.date||today}))];
            }
            // Meals: append
            if(p.health.mealLog?.length){
              const existing=curHealth.mealLog||[];
      ...
      const msg = e?.message?.includes("rate") || e?.message?.includes("429")
        ...
        : "I had a momentary issue. Please try sending that again.";
    ...
    <div style={{maxWidth:660,margin:"0 auto",width:"100%",padding:"24px 16px"}}>
      ...
      <div style={{display:"flex",flexDirection:"column",gap:10}}>
        {active.map(r=>{
          ...
          const statusColor=status==="overdue"?C.danger:status==="today"?C.warn:C.textDim;
          const statusLabel=status==="overdue"?"Overdue":status==="today"?"Today":"Upcoming";
          return<Card key={r.id} style={{display:"flex",alignItems:"center",gap:14,borderColor:status==="overdue"?C.dangerSoft:status==="today"?C.warnSoft:C.border}}>
          <button onClick={()=>toggle(r.id)} style={{width:28,height:28,borderRadius:"50%",border:`2px solid ${statusColor}`,background:"transparent",cursor:"pointer",flexShrink:0,display:"flex",alignItems:"center",justifyContent:"center"}}/>
          <div style={{flex:1}}>
            ...
            <div style={{display:"flex",gap:6,alignItems:"center",flexWrap:"wrap"}}>
              ...
              <span style={{fontSize:10,fontWeight:600,color:statusColor,padding:"2px 6px",background:statusColor+"22",borderRadius:99}}>{statusLabel}</span>
...
function HealthView({ mem, save }) {
  ...
  const [sugarCtx, setSugarCtx] = useState("fasting");
  ...
  return (
    <div style={{ maxWidth: 660, margin: "0 auto", width: "100%", padding: "24px 16px" }}>
      ...
      {!h.enabled && (
        <div style={{marginBottom:14,padding:"12px 16px",background:C.pinkSoft,borderRadius:12,border:`1px solid ${C.pink}`,display:"flex",alignItems:"center",justifyContent:"space-between",gap:12}}>
          <p style={{margin:0,fontSize:13,color:C.pink}}>Enable health tracking to get nudges, quiet hours, and AI observations.</p>
          <Btn variant="primary" onClick={() => saveH({ enabled: true })} size="sm" icon="heart">Enable</Btn>
        </div>
      )}
      ...
      <>
        ...
        <Card style={{marginBottom:14}}>
          <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:12}}>
            <div style={{display:"flex",alignItems:"center",gap:8}}>
              <Icon name="activity" size={18} color={C.danger}/>
              <p style={{margin:0,fontSize:14,fontWeight:600}}>Blood Pressure</p>
              {(h.bpLog||[]).length>0&&<Badge color="danger">{(h.bpLog||[]).length} readings</Badge>}
            </div>
            <Btn size="sm" icon="plus" onClick={()=>setAddBP(v=>!v)}>Log</Btn>
          </div>
          {addBP&&<div style={{background:C.surface,borderRadius:10,padding:"12px 14px",marginBottom:12,border:`1px solid ${C.border}`}}>
            <div style={{display:"flex",gap:8,marginBottom:8}}>
              <div style={{flex:1}}><p style={{fontSize:11,color:C.textMuted,margin:"0 0 4px"}}>Systolic</p><input type="number" value={bpSys} onChange={e=>setBpSys(e.target.value)} placeholder="120" style={{background:C.bg,border:`1px solid ${C.border}`,color:C.text,borderRadius:8,padding:"8px 10px",fontSize:14,width:"100%",boxSizing:"border-box",fontFamily:"inherit"}}/></div>
              <div style={{flex:1}}><p style={{fontSize:11,color:C.textMuted,margin:"0 0 4px"}}>Diastolic</p><input type="number" value={bpDia} onChange={e=>setBpDia(e.target.value)} placeholder="80" style={{background:C.bg,border:`1px solid ${C.border}`,color:C.text,borderRadius:8,padding:"8px 10px",fontSize:14,width:"100%",boxSizing:"border-box",fontFamily:"inherit"}}/></div>
              <div style={{flex:1}}><p style={{fontSize:11,color:C.textMuted,margin:"0 0 4px"}}>Pulse</p><input type="number" value={bpPulse} onChange={e=>setBpPulse(e.target.value)} placeholder="72" style={{background:C.bg,border:`1px solid ${C.border}`,color:C.text,borderRadius:8,padding:"8px 10px",fontSize:14,width:"100%",boxSizing:"border-box",fontFamily:"inherit"}}/></div>
            </div>
            <div style={{display:"flex",gap:8}}>
              <Btn size="sm" variant="primary" onClick={()=>{if(!bpSys||!bpDia)return;const entry={date:today,systolic:parseInt(bpSys),diastolic:parseInt(bpDia),pulse:parseInt(bpPulse)||0,ts:Date.now()};saveH({bpLog:[...( h.bpLog||[]),entry]});setBpSys("");setBpDia("");setBpPulse("");setAddBP(false);}}>Save</Btn>
              <Btn size="sm" variant="ghost" onClick={()=>setAddBP(false)}>Cancel</Btn>
            </div>
          </div>}
            ...
            {(h.bpLog||[]).slice(-5).reverse().map((r,i)=>{
              ...
              return<div key={i} style={{display:"flex",alignItems:"center",gap:12,padding:"10px 12px",background:C.surface,borderRadius:10,border:`1px solid ${C.border}`}}>
                <div style={{flex:1}}>
                  ...
                  <p style={{margin:0,fontSize:11,color:C.textDim}}>{new Date(r.ts).toLocaleDateString("en-NG",{month:"short",day:"numeric",hour:"2-digit",minute:"2-digit"})}</p>
                </div>
                <span style={{fontSize:11,fontWeight:600,color:catColor,padding:"3px 8px",background:catColor+"22",borderRadius:99}}>{cat}</span>
              </div>;
            })}
          </div>}
          {(h.bpLog||[]).length===0&&<p style={{fontSize:12,color:C.textDim,margin:0}}>No readings yet. Log BP above or tell Nia "my BP is 120/80" in chat.</p>}
        </Card>
        ...
        <Card style={{marginBottom:14}}>
          <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:12}}>
            <div style={{display:"flex",alignItems:"center",gap:8}}>
              <Icon name="droplet" size={18} color={C.warn}/>
              <p style={{margin:0,fontSize:14,fontWeight:600}}>Blood Sugar</p>
              {(h.sugarLog||[]).length>0&&<Badge color="warn">{(h.sugarLog||[]).length} readings</Badge>}
            </div>
            <Btn size="sm" icon="plus" onClick={()=>setAddSugar(v=>!v)}>Log</Btn>
          </div>
          {addSugar&&<div style={{background:C.surface,borderRadius:10,padding:"12px 14px",marginBottom:12,border:`1px solid ${C.border}`}}>
            <div style={{display:"flex",gap:8,marginBottom:8}}>
              <div style={{flex:2}}><p style={{fontSize:11,color:C.textMuted,margin:"0 0 4px"}}>Level (mmol/L)</p><input type="number" step="0.1" value={sugarVal} onChange={e=>setSugarVal(e.target.value)} placeholder="5.5" style={{background:C.bg,border:`1px solid ${C.border}`,color:C.text,borderRadius:8,padding:"8px 10px",fontSize:14,width:"100%",boxSizing:"border-box",fontFamily:"inherit"}}/></div>
              <div style={{flex:3}}><p style={{fontSize:11,color:C.textMuted,margin:"0 0 4px"}}>Context</p>
                <select value={sugarCtx} onChange={e=>setSugarCtx(e.target.value)} style={{background:C.bg,border:`1px solid ${C.border}`,color:C.text,borderRadius:8,padding:"8px 10px",fontSize:13,width:"100%",fontFamily:"inherit"}}>
                  <option value="fasting">Fasting</option><option value="after_meal">After meal</option><option value="random">Random</option>
                </select>
              </div>
            </div>
            <div style={{display:"flex",gap:8}}>
              <Btn size="sm" variant="primary" onClick={()=>{if(!sugarVal)return;const entry={date:today,level:parseFloat(sugarVal),unit:"mmol/L",context:sugarCtx,ts:Date.now()};saveH({sugarLog:[...(h.sugarLog||[]),entry]});setSugarVal("");setAddSugar(false);}}>Save</Btn>
              <Btn size="sm" variant="ghost" onClick={()=>setAddSugar(false)}>Cancel</Btn>
            </div>
          </div>}
          {(h.sugarLog||[]).length>0&&<div style={{display:"flex",flexDirection:"column",gap:6}}>
            {(h.sugarLog||[]).slice(-5).reverse().map((r,i)=>{
              const cat=r.context==="fasting"?(r.level>=7?"High":r.level>=5.6?"Pre-diabetic":"Normal"):(r.level>=11.1?"High":r.level>=7.8?"Elevated":"Normal");
              const catColor=cat==="High"?C.danger:cat==="Elevated"||cat==="Pre-diabetic"?C.warn:C.success;
              return<div key={i} style={{display:"flex",alignItems:"center",gap:12,padding:"10px 12px",background:C.surface,borderRadius:10,border:`1px solid ${C.border}`}}>
                <div style={{flex:1}}>
                  <p style={{margin:"0 0 2px",fontSize:15,fontWeight:700,color:C.text}}>{r.level} <span style={{fontSize:12,fontWeight:400,color:C.textMuted}}>mmol/L · {r.context.replace("_"," ")}</span></p>
                  <p style={{margin:0,fontSize:11,color:C.textDim}}>{new Date(r.ts).toLocaleDateString("en-NG",{month:"short",day:"numeric",hour:"2-digit",minute:"2-digit"})}</p>
                </div>
                <span style={{fontSize:11,fontWeight:600,color:catColor,padding:"3px 8px",background:catColor+"22",borderRadius:99}}>{cat}</span>
              </div>;
          ...
          {(h.sugarLog||[]).length===0&&<p style={{fontSize:12,color:C.textDim,margin:0}}>No readings yet. Log above or tell Nia "my sugar is 5.5 mmol fasting" in chat.</p>}
        </Card>

        {/* Health Report */}
        {((h.bpLog||[]).length>0||(h.sugarLog||[]).length>0)&&<Card style={{marginBottom:14}}>
          <div style={{display:"flex",alignItems:"center",gap:8,marginBottom:12}}>
            <Icon name="book" size={16} color={C.accent}/>
            <p style={{margin:0,fontSize:14,fontWeight:600}}>Health Report</p>
          </div>
          {!healthReport&&<Btn variant="primary" onClick={async()=>{
            setReportLoad(true);
            const bpData=(h.bpLog||[]).slice(-10).map(r=>`${r.systolic}/${r.diastolic} (${new Date(r.ts).toLocaleDateString()})`).join(", ");
            const sugarData=(h.sugarLog||[]).slice(-10).map(r=>`${r.level}mmol/L ${r.context} (${new Date(r.ts).toLocaleDateString()})`).join(", ");
            const prompt=`Generate a brief health summary report for ${mem.userName||"this user"}. Data: BP readings: ${bpData||"none"}. Blood sugar: ${sugarData||"none"}. Water today: ${h.waterToday||0}L of ${h.waterGoal}L goal. Medications: ${(h.medications||[]).map(m=>m.name).join(", ")||"none"}. Be warm, clear, and flag any concerning patterns. Under 150 words.`;
            const r=await ai([{role:"user",content:prompt}],"You are Nia, a caring AI health companion. Generate a warm, clear health summary. Flag concerning readings. Not a doctor — always recommend consulting one for medical decisions.");
            setHealthReport(r);setReportLoad(false);
          }} loading={reportLoad} icon="sparkle">Generate my health report</Btn>}
          {healthReport&&<>
            <p style={{fontSize:14,lineHeight:1.75,color:C.text,margin:"0 0 12px"}}>{healthReport}</p>
            <div style={{display:"flex",gap:8}}>
              <Btn size="sm" variant="ghost" onClick={()=>setHealthReport("")} icon="refresh">Regenerate</Btn>
              <Btn size="sm" variant="ghost" onClick={()=>{const b=new Blob([`HEALTH REPORT - ${new Date().toLocaleDateString()}

${healthReport}`],{type:"text/plain"});const a=document.createElement("a");a.href=URL.createObjectURL(b);a.download="nia-health-report.txt";a.click();}} icon="download">Download</Btn>
            </div>
          </>}
        </Card>}
